...
[Qset]=TASKtrajectoryGeneration(viaFeeder,Qset);
[Qset]=TASKtrajectoryGeneration(Feeder,Qset);
[Qset]=TASKtrajectoryGeneration(viaFeeder,Qset);

[Qset]=TASKtrajectoryGeneration(via1,Qset);
[Qset]=TASKtrajectoryGeneration(chip1,Qset);
[Qset]=TASKtrajectoryGeneration(via1,Qset);
...
[Qset]=TASKtrajectoryGeneration(via2,Qset);
[Qset]=TASKtrajectoryGeneration(chip2,Qset);
[Qset]=TASKtrajectoryGeneration(via2,Qset);
...
[Qset]=TASKtrajectoryGeneration(via3,Qset);
[Qset]=TASKtrajectoryGeneration(chip3,Qset);
[Qset]=TASKtrajectoryGeneration(via3,Qset);
...
[Qset]=TASKtrajectoryGeneration(via4,Qset);
[Qset]=TASKtrajectoryGeneration(chip4,Qset);
[Qset]=TASKtrajectoryGeneration(via4,Qset);

[Qset]=TASKtrajectoryGeneration(reference,Qset);
...
% animation
for i=1:length(Qset)
    Robot_263B.plot([0,Qset(i,:),0],'workspace', [-1000 1000 -1000 1000 0 1000])

end
pause(1)
totalFrame=length(Qset);
%totalTime=totalFrame*20;


%figure(2)
%qplot([zeros(length(Qset),1),dq(:,1:3),dq(:,4)/10,zeros(length(q),1)])
...
function [Qset]=TASKtrajectoryGeneration(goal,Qset)
global currentP
[p]=TASKtrajectGen(goal);
Qs=zeros(size(p));
for i = 1:length(p)
    [Qs(i,1),Qs(i,2),Qs(i,3),Qs(i,4)]=IKrobot(p(i,:));

end
%Qs=TASKIK(p); %array of t1 t2 t3 d4
%[t1,t2,t3,d4]=IKrobot(goal);% input z-axis rotation degree and px py pz
%[q_trj,dq_trj,ddq_trj]=RtrajectGen(t1, t2, t3, d4);
...
function [p_full]=TASKtrajectGen(goal)
global currentP
q0 = currentP;
qf = goal;
%S=max(abs(qf-q0)); % calculate maximum rotational joint displacement
S=abs(qf-q0);
maxAr=100; % maximum acceleration
maxAp=150;
t0=0;
maxAcc=[maxAr,maxAp,maxAp,maxAp];
tf=max(2*sqrt(S./maxAcc));

%tf=2*sqrt(S/maxA); % calculate total time from joint displacement
tf = round(10*tf)/10; % rounding up to simplify calculation
